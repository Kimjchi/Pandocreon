...
public class StrategieMoyen implements Strategie {
	...
	public int sélectionnerAction(IA ia) {
		int choix;
		if (this.peutPoserCroyant(ia) && MilieuDeTable.getInstance().getListeCroyants().size() < 10) {
			choix = 1;
		} else if (this.peutPoserGuide(ia) && ia.getNbGuidesSprituels() < 3) {
			choix = 2;
		} else if (this.peutPoserApocalypse(ia)) {
			choix = 9;
		} else if (this.aApocalypseDansSaMain(ia) & Partie.getInstance().getNumTour() > 100
				&& Partie.getInstance().getNumTour() % 100 != 0) {
			choix = 9;
		} else if (ia.isPeutSacrifierCroyant() && ia.getNbCroyants() > 4) {
			choix = 10;
		} else if (ia.isPeutSacrifierGuide() && ia.getnbPrières() > 8) {
			choix = 11;
		} else if (this.peutPoserDeuxEx(ia) && ia.getnbPrières() < 5) {
			choix = 12;
			// } else if
			// (ia.getDivinité().getLaCapaciteEstUtilisee()
			// == false
			// && ia.getnbPrières() < 3 &&
			// Partie.getInstance().getNumTour() > 10) {
			// choix = 13;
		} else
	...
	public void voulezVousDefausser(IA ia) {
		...
		if (Partie.getInstance().getNumTour() % 20 != 0 && Partie.getInstance().getNumTour() != 0) {
	...
	public boolean verifierPointsSansDécrément(IA ia, CarteAction carte) {
		return carte.estJouable(ia);
	...
	public void poserCroyant(IA ia) {
		...
		if (peutJouer) {
			...
			carte.setPosition(Position.Centre);
		}
	...
	public void sacrifierCroyant(IA ia) {
		if (ia.peutSacrifierCroyant) {
			int numGuide = 0;
			int numCroyant = 0;
			if (ia.getCarteDevant().get(numGuide).getCroyantAttache().get(numCroyant) != null) {
	...
	public int choisirAutreCroyant(int adversaire, int guide1, int guide2, int croyant) {
		...
		if (adversaire != -1 && guide1 != guide2) {
			croyant2 = 0;
		} else if (adversaire != -1 && Partie.getInstance().getJoueurs().get(adversaire).getCarteDevant().get(guide1)
				.getNbCroyantsAttachés() > 1) {
			croyant2 = 1;
		}
		return croyant2;
	}
